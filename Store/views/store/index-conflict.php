...
<div class='grid-filters managements'>
    ...
    <table>
        <tr>
                ...
                <select id="mer_label" name="search_merchant_id" class="reset-dropdown select2-dropdown">
                    ...
                    <?php if (isset($merchant_list) && !empty($merchant_list)): ?>
                        <?php foreach ($merchant_list as $merchant): ?>
                            <?php
                            $selected3 = "";
                            if ($this->input->post('search_merchant_id') == $merchant['id']) {
                ...
                <select name="search_region_id" id="region_id" class="reset-dropdown select2-dropdown">
                    ...
                    <?php if (isset($region_list) && !empty($region_list)): ?>
                        <?php foreach ($region_list as $region): ?>
                            <?php
                            $selected = "";
                            if ($this->input->post('search_region_id') == $region['region_id']) {
                ...
                <select name="search_state_id" id="state_id" class="reset-dropdown select2-dropdown">
                    ...
                    <?php if (isset($state_list) && !empty($state_list)): ?>
                        <?php foreach ($state_list as $state): ?>
                            <?php
                            $selected2 = "";
                            if ($this->input->post('search_state_id') == $state['state_id']) {
            ...
            <td>
                <select id="category" name='category' class='category-dropdown reset-dropdown category-dp topposition'
...
>>>>>>> .r384
                ...
                echo form_dropdown3('date_range_filter', $options, set_value('date_range_filter', $this->input->post("date_range_filter")), 'Country Name' . lang('bf_form_label_required'), " id='date_range_filter2' class='te-selects' ");                ?>
            </td>
            <td>
                <?php
                $style = "style='display:none;'";
                if ($this->input->post("date_range_filter") == 4) {
                    $style = "style='display:block;'";
                }
                ?>
                <div id="to_from_div" class="top_bar_date_sel" <?php echo $style; ?> >
                    &nbsp;<input type="text" class="te-date" id="from_date2" name="from_date"
                                 value="<?php echo set_value('from_date', $this->input->post("from_date")) ?>"
                                 placeholder="From Date"/>
                    <input type="text" class="te-date" id="to_date2" name="to_date"
                           value="<?php echo set_value('to_date', $this->input->post("to_date")) ?>"
                           placeholder="To Date"/>
                </div>
            </td>
            <td>
                &nbsp;
                <button type='button' class='btn find-filters topposition_botton' title='Find' data-original-title=''>
            ...
            <td>
                <?php if ($this->auth->has_permission('Store_Management.Store.Delete')) : ?>
                    <button type='button' class='btn topposition_botton' title='Delete'
                            data-original-title=''>
    ...
    <?php if (isset($records) && is_array($records) && count($records)) : ?>
        ...
        <?php foreach ($records as $record) : ?>
            <tr>
                ...
                <td>
                    <input id="store-zipcode<?php echo $counter ?>" type="text" name="zip" maxlength="6"
...
<?php if (!isset($ajax)) : ?>
    ...
    $(function () {
        ...
        $('.admin-box').delegate('.store-region', 'change', function () {
            var $parent = $(this).parents('tr');
            var $filter = {country_id: $parent.find('select.store-country').val(), region_id: $(this).val()};
            setStates($parent.find('select.store-state'), $filter, 'initState');
        });

        $('.admin-box').delegate('.store-state', 'change', function () {
            var $parent = $(this).parents('tr');
            var $filter = {country_id: $parent.find('select.store-country').val(), region_id: $parent.find('select.store-region').val(), state_id: $(this).val()};
        ...
        var country_id = $("#country_id_topsss").val();
